...
/*
//Eebel  END
uint16_t servo_open[12];			// pulse value in ms, -1 is no pulse
uint16_t servo_close[12];			// pulse value in ms, -1 is no pulse
//Eebel END
*/
...
/*
	//Eebel Start
	//Open Values
	servo_open[0]= SERVO1_OPEN;
	servo_open[1]= SERVO2_OPEN;
	servo_open[2]= SERVO3_OPEN;
	servo_open[3]= SERVO4_OPEN;
	servo_open[4]= SERVO5_OPEN;
	servo_open[5]= SERVO6_OPEN;
	servo_open[6]= SERVO7_OPEN;
	servo_open[7]= SERVO8_OPEN;
	servo_open[8]= SERVO9_OPEN;
	servo_open[9]= SERVO10_OPEN;
	servo_open[10]= SERVO11_OPEN;
	servo_open[11]= SERVO12_OPEN;

	//Close values
	servo_close[0]= SERVO1_CLOSE;
	servo_close[1]= SERVO2_CLOSE;
	servo_close[2]= SERVO3_CLOSE;
	servo_close[3]= SERVO4_CLOSE;
	servo_close[4]= SERVO5_CLOSE;
	servo_close[5]= SERVO6_CLOSE;
	servo_close[6]= SERVO7_CLOSE;
	servo_close[7]= SERVO8_CLOSE;
	servo_close[8]= SERVO9_CLOSE;
	servo_close[9]= SERVO10_CLOSE;
	servo_close[10]= SERVO11_CLOSE;
	servo_close[11]= SERVO12_CLOSE;
	//Eebel End
*/
	...
	for(i=0; i<SERVO_NUM; i++)
	{
		...
		//maybe elike this?
		//set_bit(servo_DDR(*servo_port[i]), servo_pin[i], servo_open[i], servo_close[i]);
	}
